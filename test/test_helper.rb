...
class ActiveSupport::TestCase
  ...
  def self.should_allow_key_based_auth(http_method, url, parameters={}, options={})
    ...
    context "should allow key based auth using X-RomeoProject-API-Key header for #{http_method} #{url}" do
      setup do
        @user = User.generate_with_protected!(:admin => true)
        @token = Token.generate!(:user => @user, :action => 'api')
        send(http_method, url, parameters, {'X-RomeoProject-API-Key' => @token.value.to_s})
      end
