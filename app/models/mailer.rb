...
class Mailer < ActionMailer::Base
  ...
  def initialize_defaults(method_name)
    ...
    # Common headers
    headers 'X-Mailer' => 'RomeoProject',
            'X-RomeoProject-Host' => Setting.host_name,
            'X-RomeoProject-Site' => Setting.app_title,
            'Precedence' => 'bulk',
            'Auto-Submitted' => 'auto-generated'
  end

  # Appends a Redmine header field (name is prepended with 'X-RomeoProject-')
  def redmine_headers(h)
    h.each { |k,v| headers["X-RomeoProject-#{k}"] = v }
  end
  ...
  def self.message_id_for(object)
    ...
    hash = "Romeoproject.#{object.class.name.demodulize.underscore}-#{object.id}.#{timestamp.strftime("%Y%m%d%H%M%S")}"
    host = Setting.mail_from.to_s.gsub(%r{^.*@}, '')
    host = "#{::Socket.gethostname}.Romeoproject" if host.empty?
    "<#{hash}@#{host}>"
  end
