...
class ApplicationController < ActionController::Base
  ...
  def require_login
    if !User.current.logged?
      ...
      respond_to do |format|
        ...
        format.xml  { head :unauthorized, 'WWW-Authenticate' => 'Basic realm="RomeoProject API"' }
        format.js   { head :unauthorized, 'WWW-Authenticate' => 'Basic realm="RomeoProject API"' }
        format.json { head :unauthorized, 'WWW-Authenticate' => 'Basic realm="RomeoProject API"' }
      end
      return false
    end
    true
  ...
  def api_key_from_request
    if params[:key].present?
      params[:key]
    elsif request.headers["X-RomeoProject-API-Key"].present?
      request.headers["X-RomeoProject-API-Key"]
    end
  end
